...
const MyClaims = () => {
  ...
  const filterClaims = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (claim) =>
          ...
          claim.reports?.category
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          claim.reports?.location
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase()),
      );
    }
    ...
    if (categoryFilter !== "all") {
      filtered = filtered.filter(
        (claim) => claim.reports?.category === categoryFilter,
      );
    }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "completed":
        return <span className="myclaims-status completed">Completed</span>;
      default:
  ...
  const uniqueCategories = [
    "all",
    ...new Set(claims.map((c) => c.reports?.category).filter(Boolean)),
  ];
    ...
    <div className="myclaims-container">
        ...
        <div className="myclaims-header-right">
          <div className="myclaims-search-box">
            <img
              src={searchIcon}
              alt="search"
              className="myclaims-search-icon-img"
            />
            <input
              type="text"
          ...
          <div className="myclaims-category-dropdown" ref={dropdownRef}>
            ...
            >
              ...
              <img
                src={downArrowIcon}
                alt="dropdown"
                className="myclaims-dropdown-arrow-img"
              />
            </button>
            {isDropdownOpen && (
              ...
              <div key={claim.id} className="myclaims-table-row">
                <div className="myclaims-col-item">
                  ...
                  <div className="myclaims-item-info">
                    <div className="myclaims-item-name">
                ...
                <div className="myclaims-col-reporter">
                  <div className="myclaims-reporter-name">
                    {claim.users?.name || claim.users?.email || "You"}
                  </div>
                  <div className="myclaims-reporter-location">
                    {claim.reports?.location || "N/A"}
                  </div>
                </div>
      ...
      {showModal && selectedClaim && (
        <div className="myclaims-modal-overlay" onClick={closeModal}>
          <div
            className="myclaims-modal-container"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="myclaims-modal-header">
              <h2>Claim Details</h2>
              <button className="myclaims-modal-close" onClick={closeModal}>
                ×
              </button>
            </div>
            ...
            <div className="myclaims-modal-body">
              <div className="myclaims-modal-grid">
                <div className="myclaims-modal-field">
                  ...
                  <span className="myclaims-modal-value">
                ...
                <div className="myclaims-modal-field">
                  ...
                  <span className="myclaims-modal-value">
                    {formatDateTime(selectedClaim.claim_date)}
                  </span>
                </div>
              </div>
                ...
                return (
                  (proof.description ||
                    ...
                    <div className="myclaims-modal-full">
                      <div className="myclaims-modal-label">
                        Proof Provided:
                      </div>
                      <div className="myclaims-proof-item">
                        {proof.description && (
                          <div className="myclaims-proof-row">
                            <div className="myclaims-proof-label">
                              Description:
                            </div>
                            <div className="myclaims-proof-value">
                              {proof.description}
                            </div>
                          </div>
                        )}
              ...
              {selectedClaim.status === "rejected" && (
                <div className="myclaims-modal-full">
                  <div className="myclaims-modal-rejected">
                    ...
                    <p>
                      Your claim was rejected. Please contact the admin for more
                      information.
                    </p>
                  </div>
                </div>
            ...
            <div className="myclaims-modal-footer">
              <button className="myclaims-modal-close-btn" onClick={closeModal}>
                Close
              </button>
            </div>
          </div>
