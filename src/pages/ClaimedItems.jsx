...
const ClaimedItems = () => {
  ...
  const getStatusLabel = () => {
    switch(statusFilter) {
      ...
      case "completed": return `Completed (${completedCount})`;
            ...
            <div key={claim.id} className="claimed-item">
              <div className="claimed-item-header">
                <div className="claimed-item-badges">
                  <span className={`claimed-item-type ${claim.reports?.type}`}>
                   ...
                   <span className={`claimed-item-status ${getStatusBadgeClass(claim.status)}`}>
              ...
              <div className="claimed-item-content">
                ...
                <div className="claimed-item-info">
                  <h3 className="claimed-item-title">{claim.reports?.title || "Unknown Item"}</h3>
                  ...
                  <div className="claimed-item-detail">
                    ...
                    <span>{claim.users?.email || claim.users?.name || "Unknown"}</span>
                  </div>
                  <div className="claimed-item-detail">
        ...
        <div className="claimed-modal-overlay" onClick={closeModal}>
          <div className="claimed-modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="claimed-modal-header">
              <h2>Claim Details</h2>
              <button className="claimed-modal-close" onClick={closeModal}>×</button>
            </div>
            
            <div className="claimed-modal-body">
              <div className="claimed-modal-section">
                <div className="claimed-modal-section-title">ITEM INFORMATION</div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Item Title:</span>
                  <span className="claimed-modal-value">{selectedClaim.reports?.title || "Unknown"}</span>
                </div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Type:</span>
                  <span className="claimed-modal-value">{selectedClaim.reports?.type === "lost" ? "LOST" : "FOUND"}</span>
                </div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Category:</span>
                  <span className="claimed-modal-value">{selectedClaim.reports?.category || "N/A"}</span>
                </div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Location:</span>
                  <span className="claimed-modal-value">{selectedClaim.reports?.location || "N/A"}</span>
                </div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Description:</span>
                  <span className="claimed-modal-value">{selectedClaim.reports?.description || "No description"}</span>
                </div>
              </div>

              <div className="claimed-modal-section">
                <div className="claimed-modal-section-title">CLAIM INFORMATION</div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Claimant:</span>
                  <span className="claimed-modal-value">{selectedClaim.users?.email || selectedClaim.users?.name || "Unknown"}</span>
                </div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Claim Date:</span>
                  <span className="claimed-modal-value">{formatFullDate(selectedClaim.claim_date)}</span>
                </div>
                <div className="claimed-modal-row">
                  <span className="claimed-modal-label">Status:</span>
                  <span className="claimed-modal-value">{getStatusText(selectedClaim.status)}</span>
                </div>
              </div>

              <div className="claimed-modal-section">
                <div className="claimed-modal-section-title">PROOF PROVIDED</div>
                {(() => {
                  ...
                  return (
                    <>
                      {proof.description && (
                        <div className="claimed-modal-row">
                          <span className="claimed-modal-label">Description:</span>
                          <span className="claimed-modal-value">{proof.description}</span>
                        </div>
                      )}
                      {proof.colorBrand && (
                        <div className="claimed-modal-row">
                          <span className="claimed-modal-label">Color/Brand:</span>
                          <span className="claimed-modal-value">{proof.colorBrand}</span>
                        </div>
                      )}
                      {proof.uniqueMarks && (
                        <div className="claimed-modal-row">
                          <span className="claimed-modal-label">Marks:</span>
                          <span className="claimed-modal-value">{proof.uniqueMarks}</span>
                        </div>
                      )}
                      {!proof.description && !proof.colorBrand && !proof.uniqueMarks && (
                        <div className="claimed-modal-row">
                          <span className="claimed-modal-value">{selectedClaim.proof_of_ownership || "No proof provided"}</span>
                        </div>
                      )}
                    </>
                  );
            ...
            <div className="claimed-modal-footer">
              ...
              {(selectedClaim.status === "rejected" || selectedClaim.status === "completed") && (
                <button className="claimed-modal-close-btn" onClick={closeModal}>Close</button>
              )}
            </div>
          </div>
        </div>
